...
export default async function ModuleDetailPage({ params }: ModuleDetailPageProps) {
  ...
  // Calculate total items once instead of inline
  const totalModuleItems = sortedTopics.reduce((sum, topic) => sum + topic.items.length, 0);
      ...
      {sortedTopics.length === 0 ? (
        ...
        <>
          <div className="text-sm text-[var(--text-secondary)]">
            ...
            {totalModuleItems} total item{totalModuleItems !== 1 ? 's' : ''}
          </div>

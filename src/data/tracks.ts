...
// Pre-calculate expensive reductions that were being done on every render
export const TRACK_ITEM_COUNTS: Record<string, number> = {};
let totalItems = 0;
...
tracks.forEach(track => {
  ...
  track.modules.forEach(mod => {
    mod.topics.forEach(topic => {
      trackTotal += topic.items.length;
    });
  });
  TRACK_ITEM_COUNTS[track.id] = trackTotal;
  totalItems += trackTotal;
});

export const TOTAL_SYLLABUS_ITEMS = totalItems;

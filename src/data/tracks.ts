...
// Precompute static item counts to avoid O(N) reductions on every render
export const TOTAL_SYLLABUS_ITEMS = tracks.reduce((sum, track) =>
  sum + track.modules.reduce((mSum, mod) =>
    mSum + mod.topics.reduce((tSum, topic) => tSum + topic.items.length, 0)
  , 0)
, 0);

export const TRACK_ITEM_COUNTS = tracks.reduce((acc, track) => {

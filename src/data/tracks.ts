...
// Pre-compute static totals to avoid expensive O(n) recalculations on every render
export const TOTAL_ITEMS = tracks.reduce((sum, track) =>
  sum + track.modules.reduce((mSum, mod) =>
    mSum + mod.topics.reduce((tSum, topic) => tSum + (topic.items?.length || 0), 0)
  , 0)
, 0);

// Pre-compute track totals for O(1) lookup
export const TRACK_TOTALS = tracks.reduce((acc, track) => {

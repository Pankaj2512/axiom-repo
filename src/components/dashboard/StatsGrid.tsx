...
export function StatsGrid() {
  ...
  // Memoize performance-heavy calculations based on progress array
  // Combines multiple O(N) array filters into a single O(N) pass
      ...
      if (p.status === 'COMPLETED') {
        ...
        if (p.completedAt) {
          const completedTime = p.completedAt instanceof Date
            ...
            : new Date((p.completedAt as { seconds?: number }).seconds ? (p.completedAt as { seconds?: number }).seconds! * 1000 : p.completedAt as string | number).getTime();

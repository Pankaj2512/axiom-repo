...
// Pre-compute static tracks data outside component
...
export function StatsGrid() {
  ...
  // Memoize progress calculations to prevent O(N) recalculations
  // when unrelated contexts (streaks, revision queue) trigger re-renders
  // Note: we move this ABOVE the early return to obey Rules of Hooks
  const { completedItems, overallPercentage, thisWeekCompleted } = React.useMemo(() => {
    ...
    // Single pass to calculate all progress-related stats
    const stats = progress.reduce((acc, p) => {
      ...
      return acc;
    }, { completed: 0, thisWeek: 0 });

    const overallPercentage = TOTAL_ITEMS > 0
      ? Math.round((stats.completed / TOTAL_ITEMS) * 100)
      : 0;

    return {
      completedItems: stats.completed,
      overallPercentage,
      thisWeekCompleted: stats.thisWeek
    };
  }, [progress]);

...
// Calculate static total items once outside component
...
export function StatsGrid() {
  ...
  // Combine derived stats calculation in a single O(N) pass memoized
  const { completedItems, thisWeekCompleted, overallPercentage } = React.useMemo(() => {
    ...
    progress.forEach(p => {
      if (p.status === 'COMPLETED') {
        completed++;
        if (p.completedAt) {
          const timestamp = p.completedAt as { seconds?: number };
          const completedMs = p.completedAt instanceof Date
            ? p.completedAt.getTime()
            : timestamp.seconds
              ? timestamp.seconds * 1000
              : new Date(p.completedAt as string | number).getTime();

...
// Pre-compute static data outside component to avoid recalculation on every render
...
export function StatsGrid() {
  ...
  // Memoize expensive calculations to prevent O(N) recalculations on every render
  const { completedItems, thisWeekCompleted } = React.useMemo(() => {
    ...
    for (let i = 0; i < progress.length; i++) {
      ...
      if (p.status === 'COMPLETED') {
        completed++;
        if (p.completedAt) {
          const completedMs = p.completedAt instanceof Date
            ? p.completedAt.getTime()
            : typeof p.completedAt === 'string' || typeof p.completedAt === 'number'
            ? new Date(p.completedAt).getTime()
            : new Date((p.completedAt as { seconds?: number }).seconds ? (p.completedAt as { seconds?: number }).seconds! * 1000 : p.completedAt as unknown as string).getTime();

          if (completedMs > oneWeekAgoMs) {
            thisWeek++;
          }
        }
      }

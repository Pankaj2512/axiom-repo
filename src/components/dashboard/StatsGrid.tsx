...
// Pre-calculate total items statically since tracks data is static
...
export function StatsGrid() {
  ...
  // Memoize O(N) array filtering calculations and combine into a single pass
  const { completedItems, thisWeekCompleted } = React.useMemo(() => {
    let completed = 0;
    let recent = 0;

    // use new Date().getTime() to satisfy react-hooks/purity rules
    ...
    for (let i = 0; i < progress.length; i++) {
      ...
      if (p.status === 'COMPLETED') {
        completed++;
        if (p.completedAt) {
          const completedTime = p.completedAt instanceof Date ? p.completedAt.getTime() : new Date((p.completedAt as { seconds?: number }).seconds ? (p.completedAt as { seconds?: number }).seconds! * 1000 : p.completedAt).getTime();

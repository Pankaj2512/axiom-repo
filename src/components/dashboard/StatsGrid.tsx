...
// Pre-calculate total items outside component to prevent recalculation on every render
const STATIC_TOTAL_ITEMS = tracks.reduce((sum, track) =>
...
export function StatsGrid() {
  ...
  // Memoize progress calculations to compute in a single pass and prevent recalculation on unrelated context updates
  const { completedItems, thisWeekCompleted, overallPercentage } = React.useMemo(() => {
    ...
    return {
      ...
      overallPercentage: STATIC_TOTAL_ITEMS > 0 ? Math.round((completed / STATIC_TOTAL_ITEMS) * 100) : 0
    };
  }, [progress]);

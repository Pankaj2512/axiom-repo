...
// Pre-calculate static total items outside component to avoid redundant O(N) calculations on every render
...
export function StatsGrid() {
  ...
  // Memoize O(N) progress calculations into a single pass loop
  const stats = React.useMemo(() => {
    ...
    return {
      ...
      overallPercentage: TOTAL_ITEMS > 0 ? Math.round((completed / TOTAL_ITEMS) * 100) : 0
    };
  }, [progress]);

  const { completedItems, thisWeekCompleted, overallPercentage } = stats;

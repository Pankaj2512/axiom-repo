...
// Calculate static total items once outside the component
...
export function StatsGrid() {
  ...
  // Memoize derived progress stats to avoid O(N) recalculations on every render
  const { completedItems, thisWeekCompleted } = React.useMemo(() => {
    let completed = 0;
    let thisWeek = 0;

    // Use stable timestamp instead of new Date().getTime() in loop for rules of hooks/purity

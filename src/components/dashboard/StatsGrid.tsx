...
// Pre-calculate static total items outside component to prevent O(N) recalculation on render
...
export function StatsGrid() {
  ...
  // Memoize and combine derived progress metrics to prevent recalculation
  // on unrelated context updates (e.g. streaks or dueCards) and minimize iteration
      ...
      if (p.status === 'COMPLETED') {
        ...
        if (p.completedAt) {
          const time = p.completedAt instanceof Date
            ...
            : new Date((p.completedAt as { seconds?: number }).seconds ? (p.completedAt as { seconds?: number }).seconds! * 1000 : (p.completedAt as any)).getTime();

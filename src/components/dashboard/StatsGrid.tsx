...
// Move static derived state calculation outside component to prevent O(N) calculation on every render
...
export function StatsGrid() {
  ...
  // Combine multiple O(N) array filtering passes into a single memoized calculation
  const { completedItems, thisWeekCompleted } = React.useMemo(() => {
    let completed = 0;
    let thisWeek = 0;

    // Use new Date().getTime() instead of Date.now() to comply with react-hooks/purity
      ...
      if (p.status === 'COMPLETED') {
        ...
        if (p.completedAt) {
          const completedTime = p.completedAt instanceof Date
            ...
            : new Date((p.completedAt as { seconds?: number }).seconds ? (p.completedAt as { seconds?: number }).seconds * 1000 : p.completedAt).getTime();

          if (completedTime > oneWeekAgo) {
            thisWeek++;
          }
        }
      }
  ...
  // Hook rules require this after useMemo

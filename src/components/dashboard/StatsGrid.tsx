...
// Precompute total items since tracks data is static
...
export function StatsGrid() {
  ...
  // Calculate stats in a single pass (hook must be called before conditional return)
  const { completedItems, thisWeekCompleted } = React.useMemo(() => {
    ...
    for (const p of progress) {
      if (p.status === 'COMPLETED') {
        ...
        if (p.completedAt) {
          const completedAtTime = p.completedAt instanceof Date
          ...
          if (completedAtTime > oneWeekAgoTime) {
            thisWeek++;
          }
        }
      }

...
// Bolt Optimization: Pre-calculate static totalItems outside the component
// Prevents O(N) recalculation on every render since tracks data is static
...
export function StatsGrid() {
  ...
  // Bolt Optimization: Memoize O(N) progress filtering
  // Combines two .filter() passes into a single loop and prevents recalculation
  // when unrelated contexts (useStreaks, useRevisionQueue) trigger re-renders
  const { completedItems, thisWeekCompleted } = React.useMemo(() => {
    let completed = 0;
    let thisWeek = 0;

    const oneWeekAgoTime = new Date().getTime() - (7 * 24 * 60 * 60 * 1000);
      ...
      if (p.status === 'COMPLETED') {
        ...
        if (p.completedAt) {
          const completedTime = p.completedAt instanceof Date
            ...
            : new Date((p.completedAt as { seconds?: number }).seconds ? (p.completedAt as { seconds?: number }).seconds! * 1000 : p.completedAt as unknown as string | number).getTime();
    ...
    return { completedItems: completed, thisWeekCompleted: thisWeek };
  }, [progress]);

  const overallPercentage = TOTAL_ITEMS > 0 ? Math.round((completedItems / TOTAL_ITEMS) * 100) : 0;

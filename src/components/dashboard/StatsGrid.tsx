...
// Pre-calculate total items statically to prevent O(N) recalculations on every render
...
export function StatsGrid() {
  ...
  // Memoize derived metrics in a single O(N) loop to avoid multiple .filter() passes
  // and prevent recalculation when unrelated state/context changes
  const { completedItems, thisWeekCompleted } = React.useMemo(() => {
    let completed = 0;
    let recent = 0;

    // Satisfy react-hooks/purity by using new Date().getTime() instead of Date.now()

...
export function StatsGrid() {
  ...
  useEffect(() => {
    // Only update state if needed to prevent cascading renders
    let isMounted = true;
    if (isMounted) {
      setMounted(true);
    }
  ...
  // Use pre-computed static total
  const totalItems = TOTAL_SYLLABUS_ITEMS;

  // Memoize completed items count
  ...
  // Memoize this week's completed items count
  const thisWeekCompleted = useMemo(() => {
    ...
    return progress.filter(p => {
      ...
      const completedTime = p.completedAt instanceof Date
        ...
        : new Date(
            (p.completedAt as unknown as { seconds?: number }).seconds
              ? (p.completedAt as unknown as { seconds: number }).seconds * 1000
              : p.completedAt
          ).getTime();
      return completedTime > oneWeekAgoTime;
    }).length;
  }, [progress]);

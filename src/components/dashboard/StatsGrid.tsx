...
import { TOTAL_APP_ITEMS } from '@/data/tracks';
...
export function StatsGrid() {
  ...
  const { completedItems, overallPercentage, thisWeekCompleted } = React.useMemo(() => {
    const completedItemsCount = progress.filter(p => p.status === 'COMPLETED').length;
    const overallPercentageVal = TOTAL_APP_ITEMS > 0 ? Math.round((completedItemsCount / TOTAL_APP_ITEMS) * 100) : 0;

    const oneWeekAgo = new Date();
    oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);

    const weekCompletedCount = progress.filter(p =>
    ...
    return {
      ...
      overallPercentage: overallPercentageVal,
      thisWeekCompleted: weekCompletedCount
    };
  }, [progress]);

...
export function StatsGrid() {
  ...
  // ⚡ Bolt: Memoize expensive array filters to avoid O(N) recalculations on render
  // progress array can grow large over time
  const completedItems = React.useMemo(() => {
    return progress.filter(p => p.status === 'COMPLETED').length;
  }, [progress]);

  const overallPercentage = TOTAL_SYLLABUS_ITEMS > 0 ? Math.round((completedItems / TOTAL_SYLLABUS_ITEMS) * 100) : 0;

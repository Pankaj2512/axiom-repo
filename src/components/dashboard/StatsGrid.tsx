...
// Calculate total static items outside component to avoid recalculation on render
...
export function StatsGrid() {
  ...
  // Compute progress stats in a single pass
      ...
      if (p.status === 'COMPLETED') {
        ...
        if (p.completedAt) {
          ...
          if (p.completedAt instanceof Date) {
            time = p.completedAt.getTime();
          } else {
             const seconds = (p.completedAt as { seconds?: number }).seconds;
             if (seconds !== undefined) {
               time = seconds * 1000;
             } else {
               time = new Date(p.completedAt as unknown as string).getTime();
             }
          }

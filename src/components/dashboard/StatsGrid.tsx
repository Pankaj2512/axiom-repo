...
// Calculate total items statically outside the component
...
export function StatsGrid() {
  ...
  // Memoize derived stats to prevent recalculation when unrelated state changes
      ...
      if (p.status === 'COMPLETED') {
        ...
        if (p.completedAt) {
          const time = p.completedAt instanceof Date
            ...
            : new Date((p.completedAt as { seconds?: number }).seconds ? (p.completedAt as { seconds?: number }).seconds! * 1000 : p.completedAt as unknown as number).getTime();

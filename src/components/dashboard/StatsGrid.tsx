...
// Precompute total items from static tracks data outside component
...
export function StatsGrid() {
  ...
  // Memoize progress-dependent calculations
  const { completedItems, overallPercentage, thisWeekCompleted } = React.useMemo(() => {
    ...
    const thisWeekCount = completed.filter(p =>
      p.completedAt && (
        (p.completedAt instanceof Date ? p.completedAt.getTime() : new Date((p.completedAt as any).seconds ? (p.completedAt as any).seconds * 1000 : p.completedAt).getTime()) > oneWeekAgoTime
      )
    ).length;

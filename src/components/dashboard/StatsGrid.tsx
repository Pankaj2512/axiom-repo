...
// Pre-calculate total items as they rely on static tracks data
...
export function StatsGrid() {
  ...
  // Memoize O(N) progress calculations to prevent recalculation
  // when unrelated contexts (e.g. streaks, revision) trigger renders.
  const { completedItems, thisWeekCompleted } = React.useMemo(() => {
    const completed = progress.filter(p => p.status === 'COMPLETED');
    const itemsCount = completed.length;

    const oneWeekAgo = new Date();
    oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);

    const weekCompletedCount = completed.filter(p =>
      p.completedAt && (

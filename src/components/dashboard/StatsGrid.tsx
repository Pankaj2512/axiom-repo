...
export function StatsGrid() {
  ...
  // Compute derived state in a single O(N) pass, memoized
  const { completedItems, thisWeekCompleted, overallPercentage } = React.useMemo(() => {
    let completedCount = 0;
    let thisWeekCount = 0;

    // Use getTime to avoid react-hooks/purity errors vs Date.now()
      ...
      if (p.status === 'COMPLETED') {
        ...
        if (p.completedAt) {
          ...
          if (p.completedAt instanceof Date) {
            ...
            // Typecast safely without any
            const completedAtObj = p.completedAt as { seconds?: number };
            time = completedAtObj.seconds
              ? completedAtObj.seconds * 1000
              : new Date(p.completedAt as unknown as string).getTime();
          }

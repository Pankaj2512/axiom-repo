...
// Pre-calculate static stats to avoid O(N) recalculations on every render
...
export function StatsGrid() {
  ...
  // Calculate derived progress stats in a single loop, memoized to prevent re-running
  // when unrelated context states (like dueCards or streak) change.
  // We must call hooks before conditional early returns.
  const { completedItems, thisWeekCompleted } = React.useMemo(() => {
      ...
      if (p.status === 'COMPLETED') {
        ...
        if (p.completedAt) {
          const completedTime = p.completedAt instanceof Date
            ...
            : new Date((p.completedAt as { seconds?: number }).seconds ? (p.completedAt as { seconds?: number }).seconds! * 1000 : p.completedAt as string).getTime();
    ...
    return { completedItems: completedCount, thisWeekCompleted: thisWeekCount };
  }, [progress]);

...
import { totalTrackItems } from '@/data/tracks';
...
export function StatsGrid() {
  ...
  // Calculate stats
  // ⚡ Bolt: Using pre-calculated totalTrackItems instead of nested reduce operations
  const completedItems = progress.filter(p => p.status === 'COMPLETED').length;
  const overallPercentage = totalTrackItems > 0 ? Math.round((completedItems / totalTrackItems) * 100) : 0;

  ...
  React.useEffect(() => {
    async function calculateForecast() {
      ...
      try {
        ...
        // 2 & 3. Calculate Completed Items & Velocity in a single pass
        let compCount = 0;
        let recentCount = 0;
        const fourteenDaysAgoTime = new Date().getTime() - 14 * 24 * 60 * 60 * 1000;
          ...
          if (p.status === 'COMPLETED') {
            ...
            if (p.completedAt) {
              const time = p.completedAt instanceof Date
                ...
                : (p.completedAt as { seconds?: number }).seconds
                  ? (p.completedAt as { seconds?: number }).seconds! * 1000
                  : new Date(p.completedAt as string | number).getTime();

              if (time > fourteenDaysAgoTime) {
                recentCount++;
              }
            }
          }
        ...
        const itemsLeft = totalItems - compCount;
        ...
        const avgItemsPerDay = recentCount > 0
          ? recentCount / 14
          : 0;

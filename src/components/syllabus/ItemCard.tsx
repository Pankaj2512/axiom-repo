...
export function ItemCard({ item }: ItemCardProps) {
  ...
  const difficultyVariant = item.difficulty?.toLowerCase() as "default" | "success" | "warning" | "error" | "info" | "outline" | null | undefined;
    ...
    <Link href={`/item/${item.id}`} className="block h-full group">
      ...
      >
        ...
        <div className="flex items-start gap-3">
          ...
          <span className="text-lg flex-shrink-0" aria-hidden="true">{typeEmoji}</span>
          ...
          <div className="flex flex-shrink-0 items-center gap-1 relative" onClick={(e) => e.preventDefault()}>
            ...
            {isDropdownOpen && (
              <div
                className="absolute right-0 top-8 w-48 bg-[var(--bg-secondary)] border border-white/10 rounded-lg shadow-xl z-10 overflow-hidden animate-in fade-in slide-in-from-top-2"
                  ...
                  {lists.length === 0 ? (
                    ...
                    lists.map(list => (
                      <button
                        ...
                        className="w-full text-left px-3 py-2 text-sm text-gray-300 hover:bg-white/10 hover:text-white transition-colors truncate focus-visible:outline-none focus-visible:bg-white/10"
                      >
                        {list.name}
                      </button>
                    ))
            ...
            {item.externalUrl && (
              <a
                ...
                onClick={(e) => e.stopPropagation()}
              >
                <ExternalLink className="w-4 h-4 text-[var(--accent-secondary)]" aria-hidden="true" />
              </a>
            )}
          </div>
        </div>
